...
public class TuPianReDianFragment extends BaseFragment implements
    ...
    private void loadData(String url) {
        if (getMyActivity().hasNetWork()) {
            ...
            String result = getMyActivity().getCacheStr("TuPianReDianFragment" + currentPagte);//获取缓存
    ...
    protected void onItemClick(int position) {
        ...
        bundle.putString("photoUrl", photoModle.getSeturl());//bundle携带url
    ...
    public void getResult(String result) {
    	//设置缓存数据，key = 类名+page，value = result
        ...
        // } else {
        photoAdapter.appendList(list);//追加数据，并且通知适配器刷新数据
        // }
        listsModles.addAll(list);//把数据追加给list集合
        mListView.onBottomComplete();
    }
